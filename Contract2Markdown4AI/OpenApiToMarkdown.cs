...
public static class OpenApiToMarkdown
{
    ...
    public static async Task<int> GenerateFilesAsync(OpenApiDocument document, string outputFolder, IDictionary<string, string>? metadata = null, IProgress<int>? progress = null, IProgress<string?>? filenameProgress = null)
    {
        ...
        await foreach (var item in GenerateAsync(document, metadata, progress, filenameProgress).ConfigureAwait(false))
        {
    ...
    public static async IAsyncEnumerable<GeneratedFile> GenerateAsync(OpenApiDocument document, IDictionary<string, string>? metadata = null, IProgress<int>? progress = null, IProgress<string?>? filenameProgress = null)
    {
        if (document == null) throw new ArgumentNullException(nameof(document));
        ...
        var compGen = new ComponentGenerator(root, string.Empty, metadata, expansionCache, progress, filenameProgress);
        await foreach (var comp in compGen.GetComponentsAsync().ConfigureAwait(false))
        {
            yield return new GeneratedFile
            {
                FileName = comp.FileName,
                Content = comp.Content,
                FileType = GeneratedFileType.Component
            };
        }

        // 3. Generate Index
        var idxGen = new IndexGenerator(string.Empty, apiTitle, metadata, progress, filenameProgress);
        var idx = idxGen.GetIndex(indexEntries);
        yield return new GeneratedFile
        {
